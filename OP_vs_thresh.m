...
snr_dB = 10; % average transmit SNR in dB
R_th_values = 1:0.5:4; % SE thresholds in b/s/Hz from 1 to 4 with step 0.5
snr_th_values = 2.^R_th_values - 1; % corresponding SNR thresholds
...
h = waitbar(0, 'Computing phase shifts and optimal channel gains...');

for ss = 1:sim_times % loop over simulation trials
    waitbar(ss/sim_times, h); % Update progress bar
...
Omega_G = gamma(m_Sr + 1/2) * gamma(m_rD+1/2) / gamma(m_Sr) / gamma(m_rD) * Upsilon_G^(-1/2);
...
Omega_L = gamma(alpha_Sr+1/2)*gamma(alpha_rD+1/2) / sqrt(beta_Sr*beta_rD) / gamma(alpha_Sr) / gamma(alpha_rD);
...
h = waitbar(0, 'Approximating PDF...'); % Initialize progress bar
for kk = 1:K
    waitbar(kk/K, h); % Update progress bar
...
h = waitbar(0, 'Deriving CDF and PDF...'); % Initialize progress bar
for caseIndex = 1:size(setInd, 1)
    waitbar(caseIndex/size(setInd, 1), h); % Update progress bar
    ...
    F_Z_fit = @(x) F_Z_fit(x) + factorial(N)/prod(factorial(indices)) * prodAlp * Phi2(indices.*m_G, 1+m_G*N, -zeta_W(1:K)*x, 50);
...
%% Outage probability calculation for each SE threshold
OP_sim = zeros(1, length(R_th_values));
OP_sim_new = zeros(1, length(R_th_values));
OP_ana = zeros(1, length(R_th_values));

h = waitbar(0, 'Computing outage probability for fixed SNR = 10 dB');
for idx = 1:length(R_th_values)
    waitbar(idx/length(R_th_values), h, ['Processing SE threshold: ', num2str(R_th_values(idx)), ' b/s/Hz']);
    snr_th = snr_th_values(idx);
    OP_sim(idx) = mean(Z2_sim < snr_th/snr_dB);
    OP_sim_new(idx) = mean(Z2_sim_optimal_phase_shift < snr_th/snr_dB);
    OP_ana(idx) = F_Z2_fit(snr_th/snr_dB);
end
close(h);

%% Plotting results
figure(3);
plot(R_th_values, OP_sim, 'b+-', 'DisplayName', 'Simulation');
hold on;
plot(R_th_values, OP_sim_new, 'r--', 'DisplayName', 'Simulation with Optimal Phase Shift');
plot(R_th_values, OP_ana, 'go-', 'DisplayName', 'Analytical Approximation');
xlabel('Spectral Efficiency Threshold [b/s/Hz]');
ylabel('Outage Probability');
legend('show', 'Location', 'southwest');
title('Outage Probability vs SE Threshold at Fixed Transmit SNR = 10 dB');
